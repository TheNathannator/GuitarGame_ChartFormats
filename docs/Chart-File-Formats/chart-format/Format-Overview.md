...
- `Type Code` is a set of characters which indicates the type of event.
  ...
  - `B`: Tempo change
  - `E`: Text event
  - `H`: (Legacy) Guitar Hero 1 hand animation position. Specified in the [5-Fret Guitar page](Tracks/5-Fret-Guitar.md).
  - `N`: Note event
  - `S`: Special phrase
...
- dB refers to the Feedback Editor program.
...
| `Fretboard`            | (dB) The fretboard image to use for this song, without the file extension.       | file path |
| `MediaType`            | (dB) Type of media to display in-game?                                           | string    |
| `MusicURL`             | (dB) URL for the song audio file.                                                | string    |
| `PreviewURL`           | (dB) URL for the preview audio file.                                             | string    |
