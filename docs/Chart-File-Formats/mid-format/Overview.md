...
- [`song.ini`](../song-ini/Overview.md): Song metadata
...
No song metadata is stored within the .mid file itself. Instead, an accompanying [`song.ini` file](../song-ini/Overview.md) is used. This file typically stores things like song title and artist name, but a few properties also affect how the `notes.mid` file is processed, which are noted where relevant.
